...
namespace exdir {
  ...
  void swap_bytes(char* data, uint64_t n_elements, size_t element_size) {
    ...
    for(uint64_t i = 0; i < number_of_bytes; i += element_size) {
      if(element_size == 1) {
        ...
        std::string mssg = "Cannot swap bytes for data types of size " + std::to_string(element_size);
  ...
  std::vector<size_t> load_npy(std::filesystem::path fpath, char*& data_ptr, uint64_t& n_elements, DType& dtype, bool& c_contiguous) {
    ...
    if(magic_string[0] != '\x93' || magic_string[1] != 'N' || magic_string[2] != 'U'
      || magic_string[3] != 'M' || magic_string[4] != 'P' || magic_string[5] != 'Y') {
    ...
    while(true) {
      if(header[loc1+i] != ' ') {
        if(header[loc1+i] == ',') break;
        else f_order_string += header[loc1+i];
      }
      i++;
    } 
    if(f_order_string == "False") c_contiguous = true;
    else c_contiguous = false;
    ...
    if(header[loc1+1] == '>') data_is_little_endian = false;
    else data_is_little_endian = true;
    i = 2;
    while(true) {
      if(header[loc1+i] != ' ') {
        if(header[loc1+i] != '\'') {
  ...
  void write_npy(std::filesystem::path fpath, const char* data_ptr, uint64_t n_elements, std::vector<size_t> shape, DType dtype, bool c_contiguous) {
    ...
    if(system_is_little_endian()) header += "<";
    else header += ">";
    header += DType_to_descr(dtype) + "', ";

    // Fortran ordering
    header += "'fortran_order': ";
    if(c_contiguous) header += "False, ";
    else header += "True, ";
    ...
    if(major_version == 0x01) {
      ...
      file << ((char*)&len)[0] << ((char*)&len)[1] << ((char*)&len)[2] << ((char*)&len)[3];
    }
  ...
  DType descr_to_DType(std::string dtype) {
    if(dtype == "b1") return DType::CHAR;
    else if(dtype == "B1") return DType::UCHAR;
    else if(dtype == "i2") return DType::INT16;
    else if(dtype == "i4") return DType::INT32;
    else if(dtype == "i8") return DType::INT64;
    else if(dtype == "u2") return DType::UINT16;
    else if(dtype == "u4") return DType::UINT32;
    else if(dtype == "u8") return DType::UINT64;
    else if(dtype == "f4") return DType::FLOAT32;
    else if(dtype == "f8") return DType::DOUBLE64;
    else {
  ...
  std::string DType_to_descr(DType dtype) {
    if(dtype == DType::CHAR) return "b1";
    else if(dtype == DType::UCHAR) return "B1";
    else if(dtype == DType::INT16) return "i2";
    else if(dtype == DType::INT32) return "i4";
    else if(dtype == DType::INT64) return "i8";
    else if(dtype == DType::UINT16) return "u2";
    else if(dtype == DType::UINT32) return "u4";
    else if(dtype == DType::UINT64) return "u8";
    else if(dtype == DType::FLOAT32) return "f4";
    else if(dtype == DType::DOUBLE64) return "f8";
    else {
  ...
  size_t size_of_DType(DType dtype) {
    if(dtype == DType::CHAR) return 1;
    else if(dtype == DType::UCHAR) return 1;
    else if(dtype == DType::INT16) return 2;
    else if(dtype == DType::INT32) return 4;
    else if(dtype == DType::INT64) return 8;
    else if(dtype == DType::UINT16) return 2;
    else if(dtype == DType::UINT32) return 4;
    else if(dtype == DType::UINT64) return 8;
    else if(dtype == DType::FLOAT32) return 4;
    else if(dtype == DType::DOUBLE64) return 8;
    else {
      std::string mssg = "Unknown DType identifier.";
      throw std::runtime_error(mssg);
    }
