...
namespace exdir {
  ...
  template<class T>
  Array<T>::Array(T* data_pntr, std::vector<size_t> i_shape, DType i_dtype, bool c_cont):
    shape_{i_shape}, data_{}, dtype_{i_dtype}, n_elements_{0}, c_contiguous_{c_cont} {
  ...
  template<class T>
  Array<T>::Array(std::vector<T> i_data, std::vector<size_t> i_shape, DType i_dtype, bool c_cont):
    shape_{i_shape}, data_{i_data}, dtype_{i_dtype}, n_elements_{0}, c_contiguous_{c_cont} {
  ...
  template<class T>
  std::vector<size_t> Array<T>::shape() const {return shape_;}

  template<class T>
  size_t Array<T>::size() const {return n_elements_;}
  ...
  size_t Array<T>::c_contiguous_index(std::vector<size_t>& indicies) {
    ...
    for(size_t k = 1; k <= shape_.size(); k++) {
      size_t product = 0;
      if(k == shape_.size()) product = 1;
      else {
        for(size_t l = k+1; l <= shape_.size(); l++) {
          if(product == 0) product = shape_[l-1];
          else product *= shape_[l-1];
        }
      }
  ...
  template<class T>
  const T* Array<T>::data_pointer() const {return data_.data();}

  template<class T>
  bool Array<T>::c_contiguous() const {return c_contiguous_;}

  template<class T>
  DType Array<T>::dtype() const {return dtype_;}
