...
namespace exdir {
  ...
  Dataset::Dataset(std::filesystem::path i_path): Object{i_path}, data_{nullptr},
    shape_{}, n_elements_{0}, element_size_{0}, dtype_{DType::DOUBLE64}, c_contiguous_{true} {
    if(!is_dataset()) {
  ...
  void Dataset::load_data() {
    shape_ = load_npy((path_/"data.npy"), data_, n_elements_, dtype_, c_contiguous_);
  ...
  void Dataset::write(Array<T> data) {
    write_npy(path_/"data.npy", reinterpret_cast<const char*>(data.data_pointer()), data.size(), data.shape(), data.dtype(), data.c_contiguous());
  ...
  std::vector<std::string> Dataset::member_raws() const {
    return raws_;
  }
